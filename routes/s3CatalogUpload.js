    ...
    const  upload = multer({
      storage: multerS3({
        ...
        key: function (req, file, cb) {
            cb(null, `uploads/${req.user.businessName}/${file.fieldname}_${uuidv4()}.csv`)
        },
...
s3UploadRouter.post('/upload', verifyToken, upload.single('catalog'), async function (req, res, next) {
    ...
    try {
        const newAccountDetails = await  User.findByIdAndUpdate(getUserInfo._id , {$push: {s3Catalogs: newFile}}, {
            new: true,
        }).select('-password')
